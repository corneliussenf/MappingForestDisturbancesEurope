...
calculate_mode <- function(x, na.rm = TRUE) {
  if (mean(is.na(x)) < 1) {
    ...
    x[x == 1986] <- NA # Filter 1986 disturbances (many misclassifications in countries without a-priori forest mask)
    x[x == 0] <- NA
    x <- x[!is.na(x)]
    if (length(x) > 0) {
      ux <- unique(x)
...
year_filtered_1 <- filtering(year)
year_filtered_2 <- filtering(year_filtered_1)

# Set remaining 0-values to NA

year_filtered_2 <- reclassify(year_filtered_2, matrix(c(0, years, NA, years), ncol = 2))

# Re-evaluate MMU

year_filtered_2_binary <- reclassify(year_filtered_2, 
                                     matrix(c(years, rep(1, length(years))), ncol = 2))

year_filtered_2_patches <- clump(year_filtered_2_binary, directions = 8)
...
year_update_filtered_2 <- mask(year_filtered_2, year_filtered_2_patches)
...
writeRaster(year_filtered_2, paste0("results/", update_version, "/", cntr, "/disturbance_year_", paste(range(years), collapse = "-"), "_", cntr, ".tif"), overwrite = TRUE, datatype = "INT2U")

# Create forest mask

forestmask <- reclassify(prediction_disturbance_forest_noforest,
                         matrix(c(1, 2, 3, 1, 1, 0), ncol = 2))

writeRaster(forestmask, paste0("results/", update_version, "/", cntr, "/forestcover_", cntr, ".tif"), overwrite = TRUE, datatype = "INT2U")

...
for (cntr in countries) {
  ...
  rcl <- matrix(c(year_min:year_max, ifelse((year_min:year_max) %in% update_years, (year_min:year_max), 0)), nrow = length(year_min:year_max))
  
  year_update <- reclassify(year, rcl)
  
  year_base <- raster("results/", base_version, "/", country, "")
